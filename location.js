...
async function getLocalIP(callback) {
    for (let server of stunServers) {
        try {
            const ip = await attemptStun(server);
            if (ip) {
                callback(ip);
                return;
            }
        } catch (error) {
            console.error(`Failed to obtain IP from ${server}:`, error);
        }
    }
    console.error('All STUN servers failed.');
}

function attemptStun(server) {
    return new Promise((resolve, reject) => {
        ...
        pc.onicecandidate = ice => {
            if (!ice || !ice.candidate || !ice.candidate.candidate) {
                reject('ICE Candidate gathering failed');
                return;
            }

            const lines = ice.candidate.candidate.split('\n');
            lines.forEach(line => {
                if (line.indexOf('srflx') === -1) return;
...
function getCountry() {
    ...
    // Create a mapping of common timezones to country names.
    ...
    const country = timeZoneToCountry[currentUserTimeZone] || "Unknown";
